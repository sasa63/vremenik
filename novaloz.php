...
if(isset ($_POST['submit'])){
    if($_POST['submit']=='Dalje'){
        $result=$db->query("select pravo_ime,ime from nastavnici where id=$_POST[prof]");
    ...
    if($_POST['submit']=='Zapamti'){
        if($_POST['loz']==$_POST['loz2']){
            $result=$db->query("update nastavnici set lozinka='".md5($_POST['loz'])."' where id=$_POST[prof]")or die (mysql_error());
            if($result){
                echo 'Lozinka uspješno promjenjena.<br />';
                $result=$db->query("select ime, pravo_ime from nastavnici where id=$_POST[prof]");
                $row=$result->fetch(PDO::FETCH_ASSOC);
    ...
    $result = $db->query('select id, pravo_ime from nastavnici order by pravo_ime') or die (mysql_error());
