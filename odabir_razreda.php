...
if(isset($_POST['submit']) and $_POST['submit']=='Dalje' and isset($_POST['predmet'])){
    ...
    $result = $db->query('SELECT * FROM %PREFIKS%predmeti WHERE id IN ('.implode(',', $_POST['predmet']).')') or die(mysql_error());
    while ($row=$result->fetch(PDO::FETCH_ASSOC)) {
    ...
    if(count($q)) $query = 'INSERT INTO %PREFIKS%radi_u (id_nastavnik, razred, odjel, id_predmet, id_godina) VALUE '.  implode(', ', $q);
...
if(isset($_SESSION['godina']) and !(isset($_POST['submit']) and $_POST['submit']=='Dalje')){ $id_godina= (int) $_SESSION['godina'];
    $query = 'SELECT * FROM %PREFIKS%nastavnici ORDER BY pravo_ime';
    ...
    echo '</select>';
    $query = 'SELECT * from %PREFIKS%predmeti';
    $result = $db->query($query);
