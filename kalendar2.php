...
if(isset($_POST['submit']) and $_POST['submit']=='Zapamti'){
    ...
    $db->query("update datumi SET aktivan=1 Where id_godina=$id_godina");
    if(count($neaktivni)>0) $db->query("update datumi SET aktivan=0 Where id IN ($ne_aktivni)") or die('kalendar2');
} else if(isset($_POST['Submit']) and $_POST['Submit']=='Dalje'){
    $id_godina=(int) $_POST['godina'];
    $sql="SELECT id, dan, aktivan FROM datumi WHERE id_godina = $id_godina ORDER BY dan ASC";
    $result = $db->query($sql) ;
